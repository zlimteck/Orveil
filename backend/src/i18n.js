module.exports = {
  en: {
    ...
    dispatcharrReport: (state) => ({ title: 'Dispatcharr Report', message: `Streams: ${state.activeStreams} active / ${state.streamsTotal} total | Viewers: ${state.totalViewers} | Channels: ${state.channelsTotal}\nVersion: ${state.version || '—'}` }),
    navidromeReport: (state) => ({ title: 'Navidrome Report', message: `Artists: ${state.artistCount ?? '—'} | Now playing: ${state.nowPlaying ?? 0}\nVersion: ${state.version || '—'}` }),
  ...
  fr: {
    ...
    dispatcharrReport: (state) => ({ title: 'Rapport Dispatcharr', message: `Streams : ${state.activeStreams} actifs / ${state.streamsTotal} total | Spectateurs : ${state.totalViewers} | Chaînes : ${state.channelsTotal}\nVersion : ${state.version || '—'}` }),
    navidromeReport: (state) => ({ title: 'Rapport Navidrome', message: `Artistes : ${state.artistCount ?? '—'} | En écoute : ${state.nowPlaying ?? 0}\nVersion : ${state.version || '—'}` }),
